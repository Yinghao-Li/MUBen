...
def predict(model: nn.Module,
            ...
            loss_func,
            shared_dict,
            scaler: StandardScaler = None) -> Tuple[List[List[float]], float]:
    """
    ...
    Returns
    -------
    A list of lists of predictions. The outer list is examples
    while the inner list is tasks.
    """
    model.eval()
    config.bond_drop_rate = 0
    preds = []

    # num_iters, iter_step = len(data), batch_size
    loss_sum, iter_count = 0, 0

    mol_collator = MolCollator(args=config, shared_dict=shared_dict)
...
def make_predictions(args: Config, smiles: List[str] = None):
    ...
    ----------
    args: Arguments.
    smiles: Smiles to make predictions on.
    ...
    if smiles is not None:
        ...
        test_data = get_data(path=args.data_path, config=args,
